...
# --- Ajout du mapper pour "ethereum_did" -> "did" ---
...
# --- Associer le scope "did" au client frontend ---
...
existing_users = requests.get(f"{BASE}/admin/realms/{REALM}/users", headers=H, params={"username": TEST_USER})
if existing_users.status_code == 200 and len(existing_users.json()) > 0:
    print(f"⚠️ L'utilisateur {TEST_USER} existe déjà, création ignorée.")
else:
    user_payload = {
        "username": TEST_USER,
    ...
    r = requests.post(f"{BASE}/admin/realms/{REALM}/users", headers=H, json=user_payload)
    if r.status_code == 201:
        print(f"✅ Utilisateur {TEST_USER} créé avec succès.")
    else:
        print(f"❌ Erreur lors de la création de l'utilisateur {TEST_USER} : {r.status_code} - {r.text}")
